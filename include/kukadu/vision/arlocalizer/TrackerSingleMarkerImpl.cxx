...
ARSM_TEMPL_TRACKER::init(const char* nCamParamFile, ARFloat nNearClip, ARFloat nFarClip, Logger* nLogger)
{
	if(nLogger)
		this->logger = nLogger;
...
ARSM_TEMPL_FUNC bool
ARSM_TEMPL_TRACKER::init(Camera* camera, ARFloat nNearClip, ARFloat nFarClip, Logger* nLogger)
{
	if(nLogger)
		this->logger = nLogger;
	...
	//initialize applications
	if(camera)
		setCamera(camera, nNearClip, nFarClip);
	else
		return true;
}
ARSM_TEMPL_FUNC int
