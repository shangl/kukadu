...
namespace kukadu {
	namespace skill
		{PushForward::PushForward(kukadu::StorageSingleton& storage, std::vector< KUKADU_SHARED_PTR< kukadu::Hardware > > hardware)
...
std::shared_ptr<kukadu::ControllerResult> PushForward::executeInternal() {
	auto& hardwareFactory = kukadu::HardwareFactory::get();
		...
		std::dynamic_pointer_cast<kukadu::DMPExecutor>(skill203)->setExecutionMode(kukadu::TrajectoryExecutor::EXECUTE_ROBOT);
		...
		std::dynamic_pointer_cast<kukadu::DMPExecutor>(skill203)->enableMaxForceMode(15, DMPExecutor::IGNORE_FORCE, DMPExecutor::IGNORE_FORCE, DMPExecutor::IGNORE_FORCE);
