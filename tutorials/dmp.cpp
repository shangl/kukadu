...
int main(int argc, char** args) {
    ...
    auto simLeftQueue = make_shared<KukieControlQueue>("simulation", arm + string("_arm"), *node);
    ...
    SensorStorage storage(queueVectors, std::vector<KUKADU_SHARED_PTR<GenericHand> >(), 1000);
    storage.setExportMode(SensorStorage::STORE_RBT_CART_POS | SensorStorage::STORE_RBT_JNT_POS);
    storage.startDataStorage(storeDir);
    cout << "measuerment started" << endl;

    ros::Rate r(2); r.sleep();
    simLeftQueue->jointPtp({-1.5, 1.56, 2.33, -1.74, -1.85, 1.27, 0.71});

    storage.stopDataStorage();
